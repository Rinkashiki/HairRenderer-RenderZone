...
void main() {
    ...
    for (int i = 0; i < scene.numLights; i++)
    {
        ...
        if (isInAreaOfInfluence(scene.lights[i], g_pos))
        {
            ...
            // reflective and more transmissive (see epic_hair_BSDF.glsl).
            vec3 lighting = evalEyelashBSDF(L,
                                             V,
                                             T,
                                             directFraction,
                                             NpTex,
    ...
    // SHEEN_SCALE + gloss boost, lowering `specular` alone can't tame it. Eyelashes
    // want a near-matte look, so scale it right down here (eyelash-only shader).
    const float EYELASH_SHEEN = 0.15;
    vec3 Vworld = normalize(camera.position.xyz - g_modelPos);
    color += computeEnvSheen(normalize(g_modelDir), Vworld, epicBaseColor, material.specular, material.roughness) * EYELASH_SHEEN;
